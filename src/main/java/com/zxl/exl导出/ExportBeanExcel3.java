...
/**
 * @author 胥方雁
 * @data 2018年6月5日 下午2:01:13
 */
public class ExportBeanExcel3<T> {
	/**
	...
	public HSSFWorkbook exportExcel(String title, List<String> headersName, List<String> headersId, List<T> dtoList) {
		...
		for (int i = 0; i < headersName.size(); i++) {
			// 自动列宽 y=255.86x+184.27 sheet.autoSizeColumn(i,true); + 184
		...
		Class tempClass = dtoList.get(0).getClass();
		...
		for (int i = 0; i < dtoList.size(); i++) {
			...
			for (int j = 0; j < headersId.size(); j++) {
				fieldName = headersId.get(j);
				// 比对JavaBean的属性名，一致就写入，不一致就丢弃
				if (fieldNameList.contains(fieldName)) {
					String getMethodName = null;
					if (fieldName.length() == 1) {
						// 当长度为1时,fieldName不进行截取 getMethodName = "get" +
						fieldName.toUpperCase();
					} else {
						getMethodName = "get" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1);// 拿到属性的get方法
					}
					try {
						// 通过JavaBean对象拿到该属性的get方法，从而进行操控
						Method getMethod = l.getClass().getMethod(getMethodName);
	...
	public static HttpServletResponse setResponseToExcel(HttpServletResponse response,String fileName){
		response.reset();
